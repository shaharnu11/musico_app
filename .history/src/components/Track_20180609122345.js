...
class Track extends Component {
    ...
    render() {
        ...
        if (!this.state.trackLoaded) {
            ...
            const trackBpm = <TrackBpm ref={_trackBpm => this._trackBpm = _trackBpm} bpm={trackDetails.bpm} />
            // const trackDuriationInSec = this._sound.sound.duration / 1000
            // const trackBitsPerSec = ((this._trackBpm.trackBpm === -1 ? trackDetails.bpm : this.state.trackBpm) / 60)
             const totalNumberOfBits = 12//Math.ceil(trackDuriationInSec * trackBitsPerSec) + "b"
            return (
                <div>
                    {sound}
